...
# print(df['weight'])
df['overweight'] = round(df['weight']/ ((df['height'] / 100) * (df['height'] / 100)), 1)
...
def draw_cat_plot():
    ...
    df_cat = df.melt(id_vars='cardio', value_vars=columns)
...
def draw_heat_map():
    ...
    sns.heatmap(corr, mask=mask, vmax=.28, center=0, annot=True,
